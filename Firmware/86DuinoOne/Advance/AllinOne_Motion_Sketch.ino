...
void walking(bool refresh) {
  static int section = 0;
  if(refresh == true) section = 0;
...
void loop()
{
  if(Serial1.available()){ Serial1_Command = Serial1.read(); }

  if(Serial1_Command == 'U')
  {
	if(pre_cmd != Serial1_Command) walking(true); else walking(false);
  }
  else if(Serial1_Command == 'L')
  {
    if(pre_cmd != Serial1_Command) left(true); else left(false);
  }
  else if(Serial1_Command == 'R')
  {
    if(pre_cmd != Serial1_Command) right(true); else right(false);
  }
  pre_cmd = Serial1_Command;
}
